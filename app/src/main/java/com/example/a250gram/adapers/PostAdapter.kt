package com.example.a250gram.adapers
...
import com.example.a250gram.Models.Post
import com.example.a250gram.Models.User
import com.example.a250gram.R
import com.example.a250gram.databinding.PostRvBinding
...
class PostAdapter(var context: Context,var postList: ArrayList<Post>):RecyclerView.Adapter<PostAdapter.MyHolder>() {
    ...
    override fun onBindViewHolder(holder: MyHolder, position: Int) {
        ...
        Glide.with(context).load(postList.get(position).postUrl).placeholder(R.drawable.comingsoon).into(holder.binding.postImage)
          holder.binding.time.text=postList.get(position).time
        holder.binding.csption.text=postList.get(position).caption
        holder.binding.like.setOnClickListener {
            holder.binding.like.setImageResource(R.drawable.lovealwayswins)
        }
    }
}
//        try {
